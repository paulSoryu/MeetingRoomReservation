...
│   ├── Bookings/           Booking aggregate + ValueObjects (BookingPeriod, RowVersion)
...
├── Configurations/            EF Core Fluent API entity configurations (RowVersion concurrency token here)
...
**Resource** — aggregate root representing a bookable meeting room: name, and a fixed set of bookable time slots. Create/update/remove restricted to `Admin`.

**Booking** — created from a `Resource`, a `TimeSlot`, and the requesting user. Carries a `RowVersion` concurrency token. A resource's slot can have at most one active booking at a time.

**Concurrency control**: **optimistic concurrency** via a `RowVersion` (`rowversion`/`timestamp`) column on `Booking`/slot state.

- On booking creation, EF Core includes the row version in the `WHERE` clause of the update/insert check; if two requests race for the same slot, the loser gets a `DbUpdateConcurrencyException`.
- The handler catches this and returns a `Result` carrying a `ConflictError`, which the controller maps to **`409 Conflict`** — never a silent overwrite, never an unhandled `500`.
- Deliberately **not** using a naive "check if free → then insert" two-step process — that is exactly the race condition this design prevents.
- Deliberately **not** using pessimistic locking (`SELECT ... WITH (UPDLOCK, HOLDLOCK)` / serializable transactions): with short-lived booking requests and no long-running work between check and write, optimistic concurrency avoids holding locks across a network round-trip and scales better under contention.
