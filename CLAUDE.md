...
The Domain layer applies **tactical DDD patterns** — aggregate roots (`Resource`, `Booking`) with private constructors and invariant-protecting factory methods (`Create`/`Update` returning `Result`/`Result<T>`, so invalid states are unrepresentable), immutable value objects (`WorkingHours`, `TimeSlot`), and repository interfaces as the aggregate-level persistence boundary. It deliberately stops short of full DDD, though: the system's single most important invariant — no double-booking — is enforced at the database level via a unique index rather than in-memory by the `Booking` aggregate, since slots aren't materialized as rows until booked, so there is nothing in memory for an aggregate to check against (see **Concurrency control** below). That's a conscious trade-off of DDD purity for correctness and throughput under concurrency, not an oversight.
